class Ball {
  ...
  void checkCylinderCollision(){
     PVector v;
     for(int i = 0; i < cylindersPosition.size(); i++){
       v = cylindersPosition.get(i);
       
       if( ( (location.x > 0.5*v.x-20) && (location.x < 0.5*v.x+20) ) ){ // I consider the cylinder as a square here....
          if (( (location.z > 0.5*v.y-20) && (location.z < 0.5*v.y+20)) ){
             println(" COLLISION"); 
          }
       }
     }
